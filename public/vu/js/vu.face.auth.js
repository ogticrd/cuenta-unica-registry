        ...
        try {
            ...
            if (vu.face.auth.registrationFlow) {
                if(vu.face.auth.enableSelfieList===true){
                    ...
                    response = await vu.sop.api.faceRegister(vu.face.auth.userNameValue, lastPic).then((res) => {
                        console.log(res);
                    }).catch((err) => {
                        if(err.code == "932" || err.code == "1004") {
                            response = vu.sop.api.faceLogin(vu.face.auth.userNameValue, lastPic);
                            return response;
                        }
                    });
