...
const { mockHeaders, mockCreateBrowserLogoutFlow, mockUpdateLogoutFlow } =
  vi.hoisted(() => ({
    mockHeaders: vi.fn(),
  ...
  it("forwards multiple Set-Cookie headers from Ory", async () => {
    mockCreateBrowserLogoutFlow.mockResolvedValueOnce({
      data: {
        ...
        logout_url:
          "https://ory.example.test/self-service/logout?token=logout-token-456",
      },
    });
  ...
  it("returns 500 when updateLogoutFlow fails", async () => {
    ...
    mockCreateBrowserLogoutFlow.mockResolvedValueOnce({
      data: {
        ...
        logout_url:
          "https://ory.example.test/self-service/logout?token=logout-token-789",
      },
    });

    mockUpdateLogoutFlow.mockRejectedValueOnce(
      new Error("Logout execution failed"),
    );
  ...
  it("returns success when no Set-Cookie headers come from Ory", async () => {
    mockCreateBrowserLogoutFlow.mockResolvedValueOnce({
      data: {
        ...
        logout_url:
          "https://ory.example.test/self-service/logout?token=logout-token-nocookie",
      },
    });
  ...
  it("forwards the full browser cookie string to both Ory calls", async () => {
    ...
    mockCreateBrowserLogoutFlow.mockResolvedValueOnce({
      data: {
        ...
        logout_url:
          "https://ory.example.test/self-service/logout?token=token-multicookie",
      },
    });
