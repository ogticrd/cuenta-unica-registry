...
function createRegistrationFlow(nodes: UiNode[]): Pick<RegistrationFlow, "id" | "ui"> {
    return {
        id: "ory-registration-flow",
        ui: {
            action: "https://ory.example.test/self-service/registration?flow=ory-registration-flow",
            method: "POST",
            nodes,
        },
    };
...
describe("Ory SDK contract - toSession response", () => {
    it("mock session shape satisfies the Session type", () => {
        ...
        assertType<Pick<Session, "identity" | "authenticated_at" | "expires_at">>(mockSession);
...
describe("Ory SDK contract - updateRegistrationFlow response", () => {
    it("mock success shape satisfies SuccessfulNativeRegistration", () => {
        ...
        expect(mockSuccess.continue_with?.[0]?.flow?.id).toBe("verification-flow-123");
    });
    ...
    it("mock error with ui.nodes satisfies RegistrationFlow error shape", () => {
        ...
        const mockError = createRegistrationFlow([
            createInputNode({ name: "traits.email", value: "user@example.com" }, messages),
        ]);
...
describe("Ory SDK contract - SDK method signatures", () => {
    it("FrontendApi has expected method names", async () => {
        ...
        for (const method of methods) {
            expect(
                typeof ((FrontendApi.prototype as unknown as Record<string, unknown>)[method]),
            ).toBe("function");
        }
    });
    ...
    it("IdentityApi has expected method names", async () => {
        ...
        expect(
            typeof ((IdentityApi.prototype as unknown as Record<string, unknown>).listIdentities),
        ).toBe("function");
    });
});
