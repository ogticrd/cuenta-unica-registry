...
export function DeviceItem({
  ...
  const handleOpenChange = async (open: boolean) => {
    ...
    if (
      ...
      coords === undefined &&
      ipAddress &&
      ipAddress !== t("ip_unknown")
    ) {
      setIsLoadingCoords(true);
      setCoordsError(false);
      try {
        const fetchedCoords =
          await locationApiService.getCoordinates(ipAddress);
        if (fetchedCoords) {
  ...
  return (
    <div
      className={`group flex flex-col p-6 rounded-md border-2 transition-all duration-300 hover:shadow-md ${
        isCurrent
    ...
    >
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
        <div className="flex flex-col sm:flex-row items-start gap-5 flex-1 min-w-0">
          <div
            className={`flex-shrink-0 mt-1 p-3.5 rounded-xl flex items-center justify-center transition-colors ${
              isCurrent
        ...
        <div className="relative flex items-center justify-center">
          ...
          <CollapsibleTrigger asChild>
            ...
            >
              ...
              <ChevronDown
                className={`h-3 w-3 transition-transform duration-300 text-muted-foreground ${isOpen && "transform rotate-180"}`}
              />
            </Button>
          </CollapsibleTrigger>
        </div>
        <CollapsibleContent className="transition-all duration-300">
          <div className="mt-2 space-y-4 pt-4">
